...
export async function buildCarteSecteurFeatures(codeCommune, rows) {
  ...
  const voronoiFeatures = voronoiResult.features

  console.log(`Adresses totales : ${adresses.length} - Adresses trouvées : ${adressesFeatures.length} | Ratio : ${((adressesFeatures.length / adresses.length) * 100).toFixed(0)}% | Voronoi : ${voronoiFeatures.length}`)
