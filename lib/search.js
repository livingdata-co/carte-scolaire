...
const collegeLocation = new Map()

for (const feature of collegesFeatures) {
  const {codeRNE, adresseEtablissement} = feature.properties
  const {coordinates} = feature.geometry

  collegeLocation.set(codeRNE, {coordinates, adresseEtablissement})
}
...
export function getCollegeLocation(codeRNE) {
  const location = collegeLocation.get(codeRNE)
