...
async function getAdresses(codeCommune) {
  ...
  try {
    if (await fileExists(adresseFile)) {
      const cacheData = await readFile(adresseFile, {encoding: 'utf8'})
      return Papa.parse(cacheData, {delimiter: ';', header: true}).data
    }
  } catch (error) {
    console.error(error)
  }
  ...
  try {
    ...
    await writeFile(adresseFile, banData)
...
export async function buildCarteSecteurFeatures(codeCommune, rows, colleges) {
  ...
  return dissolve(featureCollection(voronoiFeatures), {propertyName: 'codeRNE'})
    ...
    .map(f => ({
      ...f,
      properties: colleges[f.properties.codeRNE] || f.properties
    }))
}
