...
export async function buildCarteSecteurFeatures(codeCommune, nomCommune, rows, colleges) {
  ...
  return dissolve(featureCollection(voronoiFeatures), {propertyName: 'codeRNE'})
    ...
    .map(f => ({
      ...f,
      properties: colleges[f.properties.codeRNE] ? {...communeProperties, ...colleges[f.properties.codeRNE]} : {...communeProperties, ...f.properties}
    }))
}
